...
fn build_and_push_images() -> Result<()> {
    ...
    for arch in &["amd64", "arm64"] {
        ...
        cmd!(sh, "podman build --platform linux/{arch} -t {tag} .")
            .run()
        ...
        cmd!(sh, "podman cp {container_id}:/app/prog ./target/glabu-{arch}")
            .run()
		...
		if osarch::current_os_arch().is_match(arch) {
            ...
            let binary_name = sh.current_dir().file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("glabu");
            
            cmd!(sh, "podman cp {container_id}:/app/prog /usr/local/bin/{binary_name}")
                .run()
